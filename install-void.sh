...
HOSTNAME="$(ask 'Hostname' 'voidlinux')"
...
BCH_PASS="$(ask 'Encryption passphrase (will be prompted at boot)' '')"
  ...
  for cmd in "$@"; do
    if ! command -v "$cmd" >/dev/null 2>&1; then
      case "$cmd" in
        ...
        mkfs.vfat|mkfs.fat)   xbps-install -Sy dosfstools ;;
        ...
        efibootmgr)           xbps-install -Sy efibootmgr ;;
        limine)               xbps-install -Sy limine ;;
        awk)                  xbps-install -Sy gawk ;;
        *) warn "Attempting to install $cmd"; xbps-install -Sy "$cmd" || true ;;
      esac
    fi
  done
}
ensure sgdisk partprobe mkfs.vfat bcachefs mount.bcachefs dracut xgenfstab xchroot blkid lsblk efibootmgr limine awk

# Verify bcachefs-tools supports zstd compression
say "Verifying bcachefs-tools supports zstd compression"
if ! bcachefs format --help 2>&1 | grep -q -- '--compression=zstd'; then
  die "bcachefs-tools here lack zstd compression. Update the live ISO or bcachefs-tools, then rerun."
fi
...
say "Writing Limine configuration (UEFI path: ESP/EFI/limine)"
...
# ===== zram swap =====
say "Configuring zram swap via zramen"

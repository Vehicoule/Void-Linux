#!/bin/bash
# Void Linux Installer — Simple 2-part layout, FS/LUKS/LVM choices, snapshots, zswap, Limine (UEFI/BIOS)
# Takes care of dependencies in the live ISO and aims for first-run success.
...
bootstrap_deps(){
  ...
  local to_install=()
  for p in "${pkgs[@]}"; do xbps-query -R "$p" >/dev/null 2>&1 || to_install+=("$p"); done
  ((${#to_install[@]})) && xbps-install -y "${to_install[@]}"
  local required=(xchroot lsblk parted mkfs.fat mkfs.ext4 mkfs.xfs mkfs.btrfs mkfs.bcachefs efibootmgr limine)
  for cmd in "${required[@]}"; do command -v "$cmd" >/dev/null 2>&1 || { log_error "Missing command: $cmd"; exit 1; }; done
  log_success "Dependencies ready"
}

gather_config(){
  log_info "Interactive configuration"
  read -p "Hostname: " HOSTNAME; HOSTNAME=${HOSTNAME:-void-custom}
  read -p "Username: " USERNAME; USERNAME=${USERNAME:-voiduser}
  ...
  while true; do
    ...
    read -s -p "Confirm root password: " ROOT_PASSWORD_CONFIRM; echo
    [[ "$ROOT_PASSWORD" == "$ROOT_PASSWORD_CONFIRM" ]] && break || log_error "Root passwords don't match"
  done

  log_info "Available disks:"
  lsblk -d -o NAME,SIZE,MODEL,TYPE
  read -p "Target disk (e.g., sda, nvme0n1): " TARGET_DISK; TARGET_DISK=${TARGET_DISK:-sda}
  [[ -b /dev/$TARGET_DISK ]] || { log_error "Disk /dev/$TARGET_DISK not found"; exit 1; }

  # Root FS choices — ensure snapshot capability across all configs
  log_info "Root filesystem options (snapshots supported):"
  echo "1) btrfs"
  echo "2) bcachefs"
  echo "3) ext4 (requires LVM)"
  echo "4) xfs  (requires LVM)"
  read -p "Choose [1-4]: " FS_CHOICE; FS_CHOICE=${FS_CHOICE:-1}
  case "$FS_CHOICE" in
    1) ROOT_FS="btrfs" ;;
    2) ROOT_FS="bcachefs" ;;
    3) ROOT_FS="ext4" ;;
  ...
  read -p "Choose [1-2]: " ENC_CHOICE; ENC_CHOICE=${ENC_CHOICE:-2}
  if [[ "$ENC_CHOICE" == "2" ]]; then
    while true; do
      read -s -p "LUKS passphrase: " ENCRYPTION_PASSPHRASE; echo
      read -s -p "Confirm passphrase: " ENCRYPTION_PASSPHRASE_CONFIRM; echo
  ...
  read -p "Choose [1-2]: " LVM_CHOICE; LVM_CHOICE=${LVM_CHOICE:-2}
  if [[ "$ROOT_FS" =~ ^(ext4|xfs)$ ]] && [[ "$LVM_CHOICE" != "2" ]]; then
    log_error "ext4/xfs require LVM for snapshots. Enabling LVM."
    LVM_CHOICE="2"
  fi

  log_info "Swap option:"
  echo "1) No swap"
  echo "2) Swap LV (if LVM) or swapfile (if not using LVM with btrfs/ext4/xfs; partition not created)"
  read -p "Choose [1-2]: " SWAP_CHOICE; SWAP_CHOICE=${SWAP_CHOICE:-2}

  log_info "Zswap option:"
  echo "1) Disable zswap"
  echo "2) Enable zswap (zstd, 20% pool)"
  read -p "Choose [1-2]: " ZSWAP_CHOICE; ZSWAP_CHOICE=${ZSWAP_CHOICE:-2}

  read -p "Timezone (e.g., Europe/Paris): " TIMEZONE; TIMEZONE=${TIMEZONE:-Europe/Paris}
  read -p "Locale (e.g., en_US.UTF-8): " LOCALE; LOCALE=${LOCALE:-en_US.UTF-8}
  read -p "Keymap (e.g., fr): " KEYMAP; KEYMAP=${KEYMAP:-fr}

  log_info "GPU configuration:"
  echo "1) NVIDIA only"
  echo "2) Hybrid (NVIDIA + Intel)"
  echo "3) None/Other"
  read -p "Select [1-3]: " GPU_CHOICE; GPU_CHOICE=${GPU_CHOICE:-1}

  log_info "Kernel selection:"
  echo "1) Mainline only"
  echo "2) Current only"
  echo "3) Both"
  read -p "Select [1-3]: " KERNEL_CHOICE; KERNEL_CHOICE=${KERNEL_CHOICE:-3}
  ...
  read -p "Type YES to proceed (this destroys data on /dev/$TARGET_DISK): " CONFIRM
  [[ "$CONFIRM" == "YES" ]] || { log_info "Cancelled"; exit 0; }

  if [[ "$TARGET_DISK" == nvme* || "$TARGET_DISK" == mmcblk* ]]; then P1_SUFFIX="p1"; P2_SUFFIX="p2"; else P1_SUFFIX="1"; P2_SUFFIX="2"; fi
}
...
setup_partitions(){
  ...
  if [[ "$LVM_CHOICE" == "2" ]]; then
    ...
    # Size policy: 100G root, rest home; optional 8G swap LV
  ...
  if [[ -n "$HOME_DEV" && "$ROOT_FS" != "btrfs" && "$ROOT_FS" != "bcachefs" ]]; then
    case "$ROOT_FS" in ext4) mkfs.ext4 -F "$HOME_DEV";; xfs) mkfs.xfs -f "$HOME_DEV";; esac
  fi

  # Swap LV or swapfile (created later if chosen and no LVM)
...
EOF
  ...
  for f in /mnt/etc/locale.conf /mnt/etc/vconsole.conf /mnt/etc/hostname /mnt/etc/hosts; do
    sed -i "s/LOCALE/$LOCALE/g; s/KEYMAP/$KEYMAP/g; s/HOSTNAME/$HOSTNAME/g" "$f"
  done
}

build_cmdline(){
  local root_id
  root_id=$(blkid -s UUID -o value "$ROOT_DEV")
...
/ Void Linux (mainline)
protocol: linux
path: boot():/vmlinuz-mainline
module_path: boot():/initramfs-mainline.img
...
EOF
  ...
  # UEFI: place EFI binaries and conf
  ...
  cp /mnt/boot/limine.conf /mnt/boot/EFI/BOOT/

  if [[ "$FIRMWARE" == "UEFI" ]]; then
    # Create NVRAM entry (non-fatal)
    local disk="/dev/$TARGET_DISK"
    local partnum
    if [[ "$BOOT" =~ ${TARGET_DISK}p([0-9]+)$ ]]; then partnum="${BASH_REMATCH[1]}";
    elif [[ "$BOOT" =~ ${TARGET_DISK}([0-9]+)$ ]]; then partnum="${BASH_REMATCH[1]}";
    else partnum="1"; fi
    efibootmgr --create --disk "$disk" --part "$partnum" --label "Void (Limine)" --loader '\EFI\limine\BOOTX64.EFI' || true
  else
    # BIOS: copy limine-bios.sys to FAT /boot and install
    cp /mnt/usr/share/limine/limine-bios.sys /mnt/boot/limine/
    chroot /mnt limine bios-install "/dev/$TARGET_DISK"
  fi
...
# timeline and pre/post-enabled
sed -i 's/TIMELINE_CREATE="no"/TIMELINE_CREATE="yes"/' /etc/snapper/configs/root
sed -i 's/TIMELINE_LIMIT_DAILY="0"/TIMELINE_LIMIT_DAILY="7"/' /etc/snapper/configs/root
sed -i 's/TIMELINE_LIMIT_WEEKLY="0"/TIMELINE_LIMIT_WEEKLY="4"/' /etc/snapper/configs/root
sed -i 's/TIMELINE_LIMIT_MONTHLY="0"/TIMELINE_LIMIT_MONTHLY="12"/' /etc/snapper/configs/root
...
# delete older snapshots beyond KEEP
...
setup_swapfile_if_needed(){
  # If no LVM but swap was requested, create a swapfile (ext4/xfs/btrfs; not recommended on bcachefs)
...
finalize(){
  log_info "Finalizing"
  generate_fstab
  # Reconfigure initramfs now that everything is in place
  xchroot /mnt /bin/bash -c 'xbps-reconfigure -fa'
...
main(){
  check_root; check_void; bootstrap_deps
  gather_config
  setup_partitions
  install_base
  configure_system
